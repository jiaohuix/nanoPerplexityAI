"""
1 Replace openai with groq
2 Replace googlesearch with serpapi
"""
...
SEARCH_TIME_LIMIT = (
    3  # Max seconds to request website sources before skipping to the next URL
)
...
LLM_MODEL = "llama3-70b-8192"  #'gpt-3.5-turbo' #'gpt-4o'
...
system_prompt_answer = (
    """You are a helpful assistant who is expert at answering user's queries"""
)
...
search_client = serpapi.Client(api_key=SERPER_KEY)
...
def parse_google_results(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        return {
            ...
            if (url := future.result()[0]) and (page_text := future.result()[1])
        }
...
def parse_serpapi_results(
    ...
    """Perform a SerpAPI search and parse the content of the top results."""
    params = {
        "engine": "google",
        "q": query,
        # "num": num_search,
...
def llm_check_search(query, file_path, msg_history=None, llm_model=LLM_MODEL):
    ...
    if re.fullmatch(r"\bns\b", cleaned_response):
        ...
        search_dic = parse_serpapi_results(cleaned_response)

        # Format search result in dic into markdown format
        search_result_md = "\n".join(
            [f"{number+1}. {link}" for number, link in enumerate(search_dic.keys())]
        )
...
def main():
    ...
    while True:
        ...
        # LLM answers
            ...
            print("search_dic:", search_dic)
            msg_history = llm_answer(query, file_path, msg_history, search_dic)
            save_path = (
                save_path or f"{query}.md"
            )  # ensure saved file has the first query as its name
